...
public class MyOwnQuery {
    ...
    public float relBoost;


    public MyOwnQuery(ArrayList queries, Directory index, Analyzer analyze, Map map, float titleBoost,float descriptionBoost,float relBoost)  {
    ...
    public void doQuery(queryTriplet query,int topN) throws IOException, ParseException {
        ...
        ArrayList<String> querytWords = query.titleWords();
        for (int i = 0; i < querytWords.size(); i++) {
            String element = querytWords.get(i);
            Query q = new QueryParser(".W", this.analyzer).parse(element.toLowerCase());
            Query q1 = new BoostQuery(q,this.titleBoost);
            queryBuilder.add(q1, BooleanClause.Occur.SHOULD);
        }

        ArrayList<String> querydWords = query.descriptionWords();
        for (int i = 0; i < querydWords.size(); i++) {
            String element = querydWords.get(i);
            Query q = new QueryParser(".W", this.analyzer).parse(element.toLowerCase());
            Query q1 = new BoostQuery(q,this.descriptionBoost);
            queryBuilder.add(q1, BooleanClause.Occur.SHOULD);
        }
        ...
        if (map.containsKey(query.number)){
            ...
            for (document d:reldocs){
                ...
                for(String element: topNs){
                    Query q = new QueryParser(".W", this.analyzer).parse(element.toLowerCase());
                    Query q1 = new BoostQuery(q,this.relBoost);
                    queryBuilder.add(q1, BooleanClause.Occur.SHOULD);
                }
            }
        }
